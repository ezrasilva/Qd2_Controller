...
# Adicionar no final do ficheiro: networks-it-uc3m/quditto/Quditto-main/qd2_controller/src/qd2_controller/Non_ideal_QKDN/math_tools.py

def estimate_single_photon_yield(yield_signal, yield_decoy, signal_intensity, decoy_intensity):
    """
    Estima o yield (taxa de deteção) para os pulsos de fotão único (Y1).
    Esta é uma fórmula simplificada para um protocolo com um estado isca.
    Implementações mais robustas podem usar múltiplos iscas e otimizações.
    """
    if decoy_intensity == 0:  # Isca de vácuo
        # A fórmula exata pode ser mais complexa, dependendo das suposições
        yield_vacuum = yield_decoy
        numerator = signal_intensity * np.exp(-signal_intensity) * (yield_signal - yield_vacuum)
        denominator = signal_intensity - decoy_intensity # Neste caso, decoy_intensity é 0
        
        # Para evitar divisão por zero ou instabilidade
        if denominator == 0: return 0.0

        y1 = numerator / denominator
        return max(0, y1) # O yield não pode ser negativo

    else: # Isca fraco (não vácuo)
        numerator = (yield_signal * np.exp(signal_intensity) - 
                     yield_decoy * np.exp(decoy_intensity) * (signal_intensity**2 / decoy_intensity**2))
        denominator = signal_intensity - (signal_intensity**2 / decoy_intensity)
        
        if denominator == 0: return 0.0
        
        y1_times_exp = numerator / denominator
        y1 = y1_times_exp * np.exp(-signal_intensity)
        return max(0, y1)

def estimate_single_photon_error_rate(qber_signal, qber_decoy, yield_signal, yield_decoy, y1):
    """
    Estima a taxa de erro quântico (QBER) para os pulsos de fotão único (e1).
    """
    if y1 == 0 or yield_signal == 0:
        return 0.0 # Não se pode estimar a taxa de erro se não houver deteções de fotão único

    # Erros totais observados no estado de sinal e isca
    total_errors_signal = qber_signal * yield_signal
    total_errors_decoy = qber_decoy * yield_decoy

    # A fórmula exata pode variar, mas conceptualmente subtrai-se a contribuição de erros
    # de estados que não são de fotão único.
    # Esta é uma aproximação.
    numerator = total_errors_signal - total_errors_decoy
    denominator = y1

    if denominator == 0: return 0.0

    e1 = numerator / denominator
    return max(0, min(0.5, e1)) # A taxa de erro está entre 0 e 0.5


def decoy_state_secure_key_rate(y1, e1, qber_signal, h_func, f_ec):
    """
    Calcula a taxa de chave segura (R) usando os parâmetros estimados pelos Decoy States.
    
    y1: Yield dos estados de fotão único
    e1: Taxa de erro dos estados de fotão único
    qber_signal: QBER observado na base Z para os estados de sinal
    h_func: A função de entropia binária de Shannon (já existente em math_tools)
    f_ec: Eficiência do protocolo de correção de erros (normalmente >= 1)
    """
    if y1 == 0:
        return 0.0

    # Termo de amplificação de privacidade
    privacy_amplification_term = h_func(e1)
    
    # Termo de correção de erros
    error_correction_term = f_ec * h_func(qber_signal)
    
    # A taxa de chave segura é aproximadamente:
    # R = Y1 * (1 - H(e1)) - Q_signal * f_ec * H(Q_signal)
    # Onde Q_signal é o ganho total (yield) do estado de sinal.
    # A fórmula exata pode ser mais complexa. Vamos usar uma versão simplificada:
    
    rate = y1 * (1 - privacy_amplification_term) - error_correction_term
